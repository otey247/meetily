    ...
    try {
      ...
      if (isCallApi && transcriptionComplete == true) {
        ...
        try {
          ...
          // Re-key live chat history from the provisional recording meeting id
          // to the final id generated by the save
          const provisionalMeetingId = sessionStorage.getItem('indexeddb_current_meeting_id');
          if (provisionalMeetingId && provisionalMeetingId !== meetingId) {
            try {
              await chatService.reassignChatHistory(provisionalMeetingId, meetingId);
            } catch (error) {
              console.warn('Failed to migrate chat history to saved meeting:', error);
            }
          }
